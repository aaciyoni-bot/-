...
<body class="min-h-screen flex flex-col text-right">
    ...
    <div id="login-screen" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background-color: #0f172a;">
        <div class="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
            ...
            <form id="login-form" onsubmit="return false;" class="space-y-4 text-right">
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">שם משתמש</label>
                    <input type="text" id="username" class="w-full p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="הזן שם משתמש" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1">סיסמה</label>
                    <input type="password" id="password" class="w-full p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" placeholder="••••••••" required>
                </div>
                ...
                <button type="submit" id="login-btn" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-blue-200 uppercase tracking-widest">כניסה</button>
            </form>
        </div>
    </div>
        ...
        <main class="flex-grow container mx-auto px-4 lg:px-8 max-w-7xl py-8">
            ...
            <div id="view-dashboard" class="view-section block">
                <div class="mb-8 text-right">
                    ...
                    <p class="text-slate-600 font-medium italic">סיכום התחשבנות גלובלי למחזור הנבחר.</p>
                </div>
            ...
            <div id="view-agents" class="view-section hidden text-right">
                ...
                <div id="agent-details-container">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-slate-200 p-6">
                        ...
                        <p class="text-xs text-slate-400 mb-4 font-medium italic">* לחץ על שם שחקן לצפייה בפרטי גישה ובפירוט המשחקים.</p>
                        <div class="overflow-x-auto">
    ...
    <script>
            ...
            },
            {
                ...
                agents: [
                    ...
                    },
                    {
                        ...
                        players: [
                            { name: "OTC 1", pnl: 126.70, fee: 173.00, games: [{d: '01/05', t: 'NLH 5/10', r: -53.34, f: 173.00}, {d: 'MTT', t: 'טורניר', r: 180.04, f: 0}] },
                    ...
                    },
                    {
                        ...
                        players: [
                            { name: "Hagaim", pnl: 700.00, fee: 160.45, games: [{d: '02/05', r: 700, f: 160.45}] }
                        ]
                    }
                ]
            }
        ...
        function renderDashboard() {
            ...
            document.getElementById('dash-stats').innerHTML = `
                <div class="bg-white p-6 rounded-3xl border shadow-sm"><span class="text-[10px] text-slate-400 font-black block mb-1">סוכנים</span><div class="text-3xl font-black">${currentCycle.agents.length}</div></div>
                <div class="bg-white p-6 rounded-3xl border shadow-sm"><span class="text-[10px] text-slate-400 font-black block mb-1 uppercase">סה"כ עמלות</span><div class="text-3xl font-black text-blue-600">${format(summaries.reduce((s,x)=>s+x.rawFees,0))}</div></div>
                <div class="bg-white p-6 rounded-3xl border shadow-sm"><span class="text-[10px] text-slate-400 font-black block mb-1 uppercase tracking-tighter">מאזן מועדון סופי</span><div class="text-3xl font-black ${summaries.reduce((s,x)=>s+x.final,0) >=0 ? 'text-emerald-600' : 'text-red-600'}">${format(summaries.reduce((s,x)=>s+x.final,0))}</div></div>`;
        ...
        function renderAgentDetails(agent) {
            ...
            document.getElementById('agent-metrics').innerHTML = `
                ...
                ${agent.name === "חיים" ? `<div class="bg-blue-600 p-4 rounded-2xl border border-blue-500 text-white shadow-lg"><span class="text-[9px] font-black block mb-1 opacity-80">עמלת רשת (עוז)</span><div class="text-xl font-black">${format(summary.referralCut)}</div></div>` : ''}
                ...
                <div class="${summary.final >= 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'} p-4 rounded-2xl border shadow-sm col-span-2 md:col-span-1"><span class="text-[9px] font-black block mb-1 uppercase">שורה תחתונה</span><div class="text-2xl font-black ${summary.final >= 0 ? 'text-emerald-600' : 'text-red-600'}">${format(summary.final)}</div></div>`;
        ...
        function renderPlayerSingleView(playerName) {
            ...
            if(!player || player.games.length === 0) {
                ...
                document.getElementById('player-view-table').innerHTML = '<tr><td colspan="3" class="py-12 text-center text-slate-300 italic font-bold">אין פעילות רשומה</td></tr>';
            } else {
                ...
                document.getElementById('player-view-table').innerHTML = player.games.map(g => `
                    <tr class="hover:bg-slate-50 transition-colors text-right font-medium">
                        <td class="py-4 px-2 text-sm font-bold text-slate-400 uppercase tracking-tighter">${g.d || '01/05'}</td>
                        <td class="py-4 px-2 text-sm text-slate-700 font-black tracking-tight">${g.t || 'סשן'}</td>
